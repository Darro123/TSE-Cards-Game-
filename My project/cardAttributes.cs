...
public class Card : MonoBehaviour
{
    ...
    public void UseAbility(GameObject source, GameObject target)
    {
        ...
        if (abilityType == "swap")
            ...
            GameLogUI.Instance.AddMessage(cardName + " has no ability.");
    }
    ...
    void SwapHands(GameObject source, GameObject target)
    {
        ...
        GameLogUI.Instance.AddMessage(source.name + " swapped hands with " + target.name);
        GameLogUI.Instance.AddMessage(source.name + " now has " + sourceHand.cards.Count + " cards");
        GameLogUI.Instance.AddMessage(target.name + " now has " + targetHand.cards.Count + " cards");
        foreach (Card c in sourceHand.cards)
            GameLogUI.Instance.AddMessage(source.name + " hand: " + c.cardName);
        foreach (Card c in targetHand.cards)
            GameLogUI.Instance.AddMessage(target.name + " hand: " + c.cardName);
    ...
    void SkipTurn(GameObject target)
    {
        ...
        GameLogUI.Instance.AddMessage(target.name + " will skip their next turn");
    }
    ...
    void PeekShadow(GameObject source)
    {
        ...
        {
            GameLogUI.Instance.AddMessage(source.name + " has no shadow cards left");
            return;
        }

        // just reveals the top card for now, UI can hook into this later
        Card topCard = sourceHand.shadowDeck[0];
        GameLogUI.Instance.AddMessage(source.name + " peeked at their shadow card: " + topCard.cardName);
    }
    ...
    void ForceReveal(GameObject target)
    {
        ...
        {
            GameLogUI.Instance.AddMessage(target.name + " has no cards to reveal");
            return;
        }

        // reveals their first card for now, UI will let the player choose later
        targetHand.cards[0].isFaceDown = false;
        GameLogUI.Instance.AddMessage(target.name + " was forced to reveal: " + targetHand.cards[0].cardName);
    }
    ...
    void StealCard(GameObject source, GameObject target)
    {
        ...
        if (targetHand.cards.Count == 0)
        {
            GameLogUI.Instance.AddMessage(target.name + " has no cards to steal");
            return;
        }
        ...
        GameLogUI.Instance.AddMessage(source.name + " stole " + stolenCard.cardName + " from " + target.name);
    }
    ...
    void ShadowSwap(GameObject source, int handIndex, int shadowIndex)
    {
        ...
        GameLogUI.Instance.AddMessage(source.name + " swapped hand card " + handIndex + " with shadow card " + shadowIndex);
    }
